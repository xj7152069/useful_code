/************update in 2022.06.14**************/
/*
    version 2.0:  1. The number of iterations is consistent
        2. The consistency of Tikhonov regularization is maintained
***********************************************/
#include <iostream>
#include <math.h>
#include <omp.h>

#include "./slantstack3d.omp.hpp"
...
void Slantstack_CG_2D(float **recoverdata, float **tauppanel, float **trace,\
 ...
 bool dorecover=false, int ncpu=1)
{
/* discription.
This function will do the local LRT over local traces .
float **trace;
...
//void Slantstack_CG_2D(float **recoverdata, float **tauppanel, float **trace,\
 ...
 float x0(0.0);
    Slantstack_CG_2D(recover,taup2d_lslrt,gather,\
            ...
            nphr,phrmin, dphr,\
            factor_L2, factor_L1, \
            iterations_num, residual_ratio,false,1);
}
void apply_GlobalLSLRT_to_CMPgather_omp(int ntrCMP, float *offset, int ns, float dt_s,\
 float **gather, int nphr, float phrmin, float dphr, int ntau,float **taup2d_lslrt,\
 float factor_L2=1.0,float factor_L1=1.0, \
 int iterations_num=45, float residual_ratio=0.1,int ncpu=1)
{
//void Slantstack_CG_2D(float **recoverdata, float **tauppanel, float **trace,\
 float *coor, int ntrace, float x0, int ns, float dt,\
 int npsr,float psrmin, float dpsr,\
 ...
 float x0(0.0);
    Slantstack_CG_2D(recover,taup2d_lslrt,gather,\
            ...
            nphr,phrmin, dphr,\
            factor_L2, factor_L1, \
            iterations_num, residual_ratio,false,ncpu);
}
