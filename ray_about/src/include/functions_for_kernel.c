...
int get_qp (float *q, float *p, ray_lamda *r_lamda, int n_point, float ds)
{
	...
	//float complex unit_imag = 1.0I;

/*----------------------Const Velocity Model-----------------------*/
	/*for (i=0; i<n_point; i++)
	{
		//q1[i] = pow(vel_s[i],0.9) * p10 * ray_length[i] + q10;
		q1[i] = vel_s[i] * p10 * ray_length[i] + q10;
		p1[i] = p10;
	}*/
...
/*-----------------------------Euler Method------------------------------*/
	...
	for (i=0; i<n_point-1; i++)
	{
		ray_length_tmp = ray_length[i+1]-ray_length[i];
		q1[i+1] = q1[i] + ray_length_tmp * (vel_s[i]*p1[i]);
		p1[i+1] = p1[i] + ray_length_tmp *
			((-1.0)/(vel_s[i]*vel_s[i])*vel_s_nn[i]*q1[i]);
	}
	q2[0] = q20;
	p2[0] = p20;
	for (i=0; i<n_point-1; i++)
	{
		ray_length_tmp = ray_length[i+1]-ray_length[i];
		q2[i+1] = q2[i] + ray_length_tmp * (vel_s[i]*p2[i]);
		p2[i+1] = p2[i] + ray_length_tmp *
			((-1.0)/(vel_s[i]*vel_s[i])*vel_s_nn[i]*q2[i]);
	}*/
/*-----------------------------------------------------------------*/
