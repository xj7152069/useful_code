...
! 29 Jun 2013: Made compliant with new DataModule
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

module lsqrCheckModule

  use  lsqrDataModule,    only : dp, ip, eps
...
!    real(dp)                :: x(n), v(n), w(m), y(m)
	real(dp),allocatable                :: x(:), v(:), w(:), y(:)
    integer(ip)             :: i, j
    real(dp)                :: alfa, beta, t, test1, test2, test3, tol
...
!print*,'Test-121'      
 ...
 1020 format(1p, &
  ...
  subroutine xcheck( m, n, Aprod1, Aprod2, b, damp, x, &
    ...
    real(dp)            :: r(m), v(n)
    real(dp)            :: bnorm, dampsq, rho1, rho2, sigma1, sigma2, &
