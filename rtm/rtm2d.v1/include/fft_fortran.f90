...
!     Perform 2D FFT with flag: INV. =1, forward; =-1, backward.
!     A2D(N1,N2)  is the input 2d buffer, overwritten by output.
!     AT2D(N1,N2) is a temporal buffer.
!     Author: FengBo, 2010-07-17.
      IMPLICIT NONE
      INTEGER N1, N2, INV
...
!     1D FFT to the slow-dimension, using the transposed buffer AT2D
!$OMP PARALLEL
!$OMP DO
      DO I1 = 1, N1
...
!===============================================================C 
!                                                               C
!   ROUTINE NAME :  FFTCC                                       C
!                                                               C
! --------------------------------------------------------      C
!                                                               C
!   PURPOSE :  COMPUTE THE FAST FOURIER TRANSFORM OF A          C
!              COMPLEX VALUED SEQUENCE                          C
!                                                               C
!   USAGE :  CALL FFTCC(A,N)                                    C
!                                                               C
!   ARGUMENTS : A    COMPLEX VECTOR OF LENGTH N,                C
!                    ON INPUT A CONTAINS THE                    C
!                    COMPLEX VALUED SEQUENCE TO BE              C
!                    ON OUTPUT A IS REPLACED BY THE             C
!                    FOURIER TRANSFORM.                         C
!               N    INPUT NUMBER OF DATA TO BY                 C
!                    TRANSFORMED. N MAY BE ANY POSITIVE         C
!                    INTEGER.                                   C
!                                                               C
!  NOTATION :   INFORMATION ON SPECIAL NOTATION AND             C
!               CONVENTIONS IS AVAILABLE IN THE MANUAL          C
!               INTRODUCTION OR THROUGH ROUTIME HELP            C
!                                                               C
!  REMARKS : 1. FFTCC COMPUTES THE FOURIER TRANSFORM,X,         C
!              ACCORDING TO THE FOLLOWING FORMULA;              C
!                                                               C
!              X(K+1)= SUM FORM J=0 TO N-1 OF                   C
!                      A(J+1)*CEXP((0.0,(2.0*PI*J*K)/N))        C
!             FOR K=0,1,...,N-1 AND PI=3.1415926...             C
!                                                               C
!             NOTE THAT X OVERWRITES A ON OUTPUT.               C
!                                                               C
!           2. FFTCC CAN BE USED TO COMPUTE                     C
!                                                               C
!             X(K+1)= (1/N)*SUM FORM J = 0 TO N-1 OF            C
!                     A(J+1)*CEXP((0.0,(-2.0*PI*J*K)/N))        C
!             FOR K=0,1,...,N-1 AND PI=3.1415926...             C 
!                                                               C
!          BY PERFORMING THE FOLLOWING STEPS;                   C
!                                                               C
!             DO 10 I=1,N                                       C
!        10   A(I)= CONJG(A(I))                                 C
!             CALL FFTCC(A,N)                                   C
!             DO 20 I=1,N                                       C
!        20   A(I)= CONJG(A(I))/N                               C
!                                                               C
! --------------------------------------------------------------C
...
!     SPECIFICATIONS FOR LOCAL VARIABLES
      REAL A0,A1,A2,A3,A4,B0,B1,B2,B3,B4,ZERO,ONE,TWO,&
        Z0(2),Z1(2),Z2(2),Z3(2),Z4(2),HALF
      COMPLEX ZA0,ZA1,ZA2,ZA3,ZA4,AK2
      EQUIVALENCE (ZA0,Z0(1)),(ZA1,Z1(1)),(ZA2,Z2(1)),&
                (ZA3,Z3(1)),(A0,Z0(1)),(B0,Z0(2)),&
                 (A1,Z1(1)),(B1,Z1(2)),(A2,Z2(1)),&
                 (B2,Z2(2)),(A3,Z3(1)),(B3,Z3(2)),&
                 (ZA4,Z4(1)),(Z4(1),A4),(Z4(2),B4)
...
!    HANDLESEPARATELY.
  ...
  215 I=1
      ...
      I=IWK(J+1)+I
  220 CONTINUE
!   THE RESULT IS NOW PERMUTED TO NORMAL ORDER.
...
!    OF LENGTH GREATER THAN OR EQUAL TO TWO.
  ...
  295 K2=IABS(IWK(ILL+K2))
      ...
      GO TO 295
  300 CONTINUE
!    REORDER A FOLLOWING THE PERMUATION CYCLES
      I=0
      J=0
      KB=0
      KN=N
