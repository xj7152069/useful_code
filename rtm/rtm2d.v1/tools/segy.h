...
typedef struct {	/* segy - trace identification header */
	...
	int fldr;	/* 3Original field record number3*/

	int tracf;	/* Trace number within original field record4*/

	int ep;		/* 5energy source point number 5
			   ---Used when more than one record occurs
			   at the same effective surface location.
			 */

	int cdp;	/* 6Ensemble number (i.e. CDP, CMP, CRP,...)6*/
	...
	int gwdep;	/* 17Water depth at receiver group. */
	...
	int  sx;	/* 19Source coordinate - X */

	int  sy;	/* Source coordinate - Y */

	int  gx;	/* 21Group coordinate - X */
