...
***********************************************************************/
void dataread_fromsegy_to3dblock_needchangepar(segyhead & head, int nline, int keyw)
{
    //keyw==1, use cdp; keyw==2, use trace;
    int i,j,k,nz(3501),ny(15),cdp0,cdp1,trac0,trac1,sx0,sx1;
    ny=nline;
    fmat numnx(ny,1,fill::zeros);
    ofstream outf,outf2;
    outf.open("./data4.bin",ios::trunc);
...
/////////////////////////////
    ...
    inf.open("./data4.bin");
    outf.open("./data4.oneblock.bin",ios::trunc);
    outf2.open("./result/data4.nx.bin",ios::app);
    for(i=0;i<ny;i++)
    {
        data.fill(0.0);
