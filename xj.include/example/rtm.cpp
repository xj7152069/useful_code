...
int main()
{
    int nz(300),nx(497),T(5000),i,j,k,sx(50),sx1(20),sx2(480);
...
for(sx=sx1;sx<sx2;sx=sx+5)
{
    ...
    out1.close();
    file[0]='\0';
    strcat(file,"../kiff_psdm/data/suf/suf1.bin");
    strcat(file,numtostr(sx,5));
    datawrite(suf,T,nx,file);
    
    s.cleardata();
    matcopy(s.model,smoothmodel,nz,nx);
    inf1.open("./data/movie1.bin");
...
datawrite(image,nz,nx,"./data/rtm.image1.bin");
Laplace(sr,image,nz,nx);
Laplace(image,sr,nz,nx);
datawrite(image,nz,nx,"./data/rtm.image1.la2.bin");
