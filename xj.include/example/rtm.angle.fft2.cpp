...
int main()
{
    int par_num(8),par_beg(50),par_gep(50),par_gep_in(5);
    int i,j;
...
void test(int par1, int par2, int par3)
{
    ...
    float **vx,**vz,**swave,**rwave,**imag,a_beg(-150.0),a_gep(3.0),na(100),a_err(20.0),*waveangle;
    int nz,nx,zfft,xfft,nwin(18);
    ...
    ofstream outffacig,outfdacig,outfimag;
    ofstream outstheta,outrtheta;

    fmat w0(Xfft,Zfft);
    w0=windowsZ(Xfft,Zfft,10.0);
    fmat w(Zfft,Xfft);
    w=w0.t();
...
while(sx<source_end)
{
    ...
    for(i=0;i<=S.PML_wide;i++)
    {
        ns.col(i).fill(0.0);
        nsh.col(i).fill(0.0);
        ns.col(X-i-1).fill(0.0);
        nsh.col(X-i-1).fill(0.0);
    }
