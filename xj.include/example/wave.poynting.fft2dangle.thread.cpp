...
void test(int par1, int par2, int par3)
{
    ...
    /////////////////////////////////angle calculation//////////////////////////////////
        ...
        file1[0]='\0';
        strcat(file1,"./data/zy.movie.down.bin");
        //strcat(file1,"./data/zy.movie.s.bin");
        strcat(file1,numtostr(source_beg,5));
        infs.open(file1);
        cout<<file1<<endl;
        file1[0]='\0';
        ...
        for(k=100;k<T-500;k++)
        {
            ...
            //infs.seekg(Z*X*(k)*sizeof(float), ios::beg);
            dataread(swave,zfft,xfft,infs);
            //dataread(swave,Z,X,infs);
            ...
            for(i=0;i<nz;i++)
            {
                for(j=0;j<nx;j++)
                {
                    if(sqrt(pow(vz[i][j],2)+pow(vx[i][j],2))<0.00000001)
                    {
                       vz[i][j]=0; 
                       vx[i][j]=0;
                    }
