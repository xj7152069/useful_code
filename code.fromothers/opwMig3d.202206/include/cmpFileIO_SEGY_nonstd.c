...
printf("ns=%d dt=%d(us) line1=%d cdp1=%d MAX_INT=%d\n", ns, dtus, line1, cdp1, MAX_INT);
    ...
    while ( 1 )
    {
        ...
        gx  = segy_hdr.gx;
        gy  = segy_hdr.gy;
//if ( 1 == verbose )   printf("itr=%ld lineno=%d cdpno=%d\n", itr, lineno, cdpno);
...
/*
    // for BN3D data.
        ...
        float   myf = 0.5*(sy+gy);
        if ( 1 == verbose )
            printf("itr=%ld lineno=%d cdpno=%d mx=%d mxf=%f my=%d myf=%f\n", itr, lineno, cdpno, mx, my, mxf, myf);
        if ( itr > 10 ) exit(1);
*/
