...
int main( int argc , char *argv[] )
{
    ...
    medianfilter(gather, ntrCMP, ns,25,1,3);
    ...
    etime   = MPI_Wtime();
    fprintf(stdout, "runtime of apply_LSLRT_to_CMPgather() is %f(s).\n", etime-stime);
    ...
    etime   = MPI_Wtime();
    fprintf(stdout, "runtime of tauph_transform_3d_to_2d_robust() is %f(s).\n", etime-stime);
    ...
    etime   = MPI_Wtime();
    fprintf(stdout, "runtime of slantStack_TD_2D_omp() is %f(s).\n", etime-stime);
    write_2d_float_wb(taup2d, nphr, ntau, "taup2d_global_lrt.dat");
    ...
    */
    ...
    // Assume the "x0" has been removed from the coordinates "halfoffset"!
    // A larger regularization "factor_L2" is helpful to the convergence in noisy data!
    stime   = MPI_Wtime();
    fprintf(stdout, "apply_GlobalLSLRT_to_CMPgather()\n");
    float factor_L2=(1000.0), factor_L1=(0.0), iterations_num=(45), residual_ratio=(0.1);
    factor_L2=1.0;
    iterations_num=20.0;
    int nthread = 16;
    apply_GlobalLSLRT_to_CMPgather_omp( ntrCMP, halfoffset, ns, dt_s,\
            gather, nphr, phrmin, dphr, ntau,taup2d_lslrt,\
            factor_L2, factor_L1,iterations_num, residual_ratio, nthread);
    etime   = MPI_Wtime();
    fprintf(stdout, "runtime of apply_GlobalLSLRT_to_CMPgather() is %f(s).\n", etime-stime);
