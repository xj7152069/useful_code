...
// Input parameters.
    ...
    int rotateCoorFlag,     // =1, rotate the coordinates.
    float   angle,          // the rotation angle, valid if rotateCoorFlag==1.
    float   coor_x0,        // the rotation center, valid if rotateCoorFlag==1.
    float   coor_y0,        // the rotation center, valid if rotateCoorFlag==1.
    ...
    )
{
    ...
    float   off_Max = 0;
    float   off_Min =1E10;
    float   coorSX_Max = -1E10;
    float   coorSX_Min = +1E10;
    float   coorSY_Max = -1E10;
    float   coorSY_Min = +1E10;
    float   coorSZ_Max = -1E10;
    float   coorSZ_Min = +1E10;
    float   coorGX_Max = -1E10;
    float   coorGX_Min = +1E10;
    float   coorGY_Max = -1E10;
    float   coorGY_Min = +1E10;
    float   coorGZ_Max = -1E10;
    float   coorGZ_Min = +1E10;
    if ( 1 == verbose )
printf("ns=%d dt=%d(us) line1=%d cdp1=%d MAX_INT=%d\n", ns, dtus, line1, cdp1, MAX_INT);
    ...
    if ( 1 == rotateCoorFlag )
    {
        cos_sita    = cos(angle*PI/180.0);
        sin_sita    = sin(angle*PI/180.0);
    }
    ...
    while ( 1 )
    {
        ...
        if ( 1 == rotateCoorFlag )
        {
            float   sx1 = cos_sita*(sx-coor_x0) - sin_sita*(sy-coor_y0) + coor_x0;
            float   sy1 = sin_sita*(sx-coor_x0) + cos_sita*(sy-coor_y0) + coor_y0;
            sx  = sx1;
            sy  = sy1;
            float   gx1 = cos_sita*(gx-coor_x0) - sin_sita*(gy-coor_y0) + coor_x0;
            float   gy1 = sin_sita*(gx-coor_x0) + cos_sita*(gy-coor_y0) + coor_y0;
            gx  = gx1;
            gy  = gy1;
        }
        ...
        printf("itr=%ld fldr=%d sx=%d sy=%d sz=%d gx=%d gy=%d gz=%d offset=%f traceid=%d tfb=%f(ms)\n",
        ...
        */
        ...
        if ( 1 == outputAcq3D_flag )
        {
            ...
            {   // the next shotgather.
                ...
                if ( sx > coorSX_Max )  coorSX_Max = sx;
                if ( sx < coorSX_Min )  coorSX_Min = sx;
                if ( sy > coorSY_Max )  coorSY_Max = sy;
                if ( sy < coorSY_Min )  coorSY_Min = sy;
                if ( sz > coorSZ_Max )  coorSZ_Max = sz;
                if ( sz < coorSZ_Min )  coorSZ_Min = sz;

                printf("fldr=%d sx=%f sy=%f sz=%f gx=%f gy=%f gz=%f offset=%f traceid=%d tfb=%f(ms)\n",
                        fldr, sx, sy, sz, gx, gy, gz, offset, kw237, tfb);

                if ( 1 == outputshot_flag )
                {
                    fprintf(shatmapFilefp, "%f\t%f\n", sx, sy);
                }
            }
            ...
            if ( gx > coorGX_Max )  coorGX_Max = gx;
            if ( gx < coorGX_Min )  coorGX_Min = gx;
            if ( gy > coorGY_Max )  coorGY_Max = gy;
            if ( gy < coorGY_Min )  coorGY_Min = gy;
            if ( gz > coorGZ_Max )  coorGZ_Max = gz;
            if ( gz < coorGZ_Min )  coorGZ_Min = gz;
    ...
    if ( 1 == outputAcq3D_flag )
    {
        ...
        fprintf(stderr, "sx_min=%f sx_max=%f\n", coorSX_Min, coorSX_Max);
        fprintf(stderr, "sy_min=%f sy_max=%f\n", coorSY_Min, coorSY_Max);
        fprintf(stderr, "sz_min=%f sz_max=%f\n", coorSZ_Min, coorSZ_Max);
        fprintf(stderr, "gx_min=%f gx_max=%f\n", coorGX_Min, coorGX_Max);
        fprintf(stderr, "gy_min=%f gy_max=%f\n", coorGY_Min, coorGY_Max);
        fprintf(stderr, "gz_min=%f gz_max=%f\n", coorGZ_Min, coorGZ_Max);
        fprintf(stderr, "offset=%f offset_x=%f offset_y=%f\n", off_Max, offX_Max, offY_Max);
    }
...
int main( int argc , char *argv[] )
{
    ...
    rotateCoorFlag     = 1;    // =1, rotate the coordinates.
    angle   = 20.742;
    coor_x0 = 487000.0;
    coor_y0 = 4870000.0;
    ntr_cmpfile_max = 20;
    printHeader_nonstd(segyFilename, cmpfileOffsetBeg, conv, endian, ntr_cmpfile_max,
        outputAcq3D_flag, acqFilename, outputshot_flag, shotmapFilename,
        rotateCoorFlag, angle, coor_x0, coor_y0, 1);
    return  mystat;
    /*
    */
