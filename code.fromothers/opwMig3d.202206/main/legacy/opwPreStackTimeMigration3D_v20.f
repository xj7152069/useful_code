...
*======================================================================*
      ...
      CALL MPI_BCAST(FN_PAR, 1024, MPI_CHARACTER, 0,
     +         MPI_COMM_WORLD, ierr)
...
C     write the common-ph imaging gathers.
     ...
     +        recl=Ntau*Nmx1)
            DO imy = 1, Nmy1
               write(IFN03, rec=imy) ((RIMAG(it,ix,iy),it=1,Ntau),
     +         ix=1,Nmx1)
            END DO
            close(IFN03)
         END IF
...
C     +              Iww, filestat, 1)
C              OPEN(66,FILE=FNW,ACCESS='DIRECT',
C    +              STATUS='OLD',RECL=NX*NY*2)
C              WRITE(66,REC=Iww)((DATW_W(IX,IY),IX=1,NX),IY=1,NY)
C              CLOSE(66)
            END IF
         ELSE
...
C     open the plane-wave gather file: FN1.
     ...
     +     recl=LT*ncdp) 
      ...
      open(78, file='/data2_2/fb/LJ2017_cmp/comphr.dat',
     +     access='direct', recl=LT*ncdp) 

C           DO imy = 1, Nmy1
C              write(IFN03, rec=imy) ((RIMAG(it,ix,iy),it=1,Ntau),
C    +         ix=1,Nmx1)
C           END DO
...
C        write(*,*) 'Iline = ', Iline
         ...
         write(78, rec=Iline-Nline_first+1) ((Pdat2d(it,ix),it=1,LT),
     +      ix=1,ncdp)
...
*======================================================================*
     ...
     +         IphrR1, IphrR2 )
      ...
      CHARACTER*1024 FN_PAR, FN1, FN2, FN3, FN4, CTMP, FNTMPW, FNTMPR
