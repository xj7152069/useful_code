...
#include "segyFileIO.cmpGather.shengli.c"
...
int main( int argc , char *argv[] )
{
	...
	// for test only.
	conv	= 1;			// = 0 ; assume data is in native format.
	endian	= 0;			// set =0 for little-endian machines(PC's,DEC,etc.).
	cmpfileOffsetBeg= 3600;		// =3600, SEGY-format.
	ntr_cmpfile	= 595430;	// trace number of file: bn_12s_line_2351.segy
	//ntr_cmpfile	= 40000;
...
*/
	...
	int	istat	= init_cmp_gather_shengli(
		cmpfileOffsetBeg, ntr_cmpfile,
		conv, endian,
		&ns, &dt, &dmx, &dmy,
		&ncdp_first, &ncdp_final,
		&nline_first, &nline_final,
		&ntr_gather_max, cmpfileidx, verbose);
	if ( 0 != istat )
	{
