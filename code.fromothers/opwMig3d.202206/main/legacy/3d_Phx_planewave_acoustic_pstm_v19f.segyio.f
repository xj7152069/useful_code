...
*======================================================================*
      open(IFN02, file=trim(adjustl(FN2)), access='direct',
     +     recl=Nvmx*Nvmy) 
...
!     parameters for writing temp files.
      ...
      CHARACTER*256 FN4, FNTMPW, FNTMPR, FNW, FNR, FNCIG
      CHARACTER*4   ciphr
      CHARACTER     c1, c2, c3
      INTEGER       fnlen
...
C    +           1, filestat, 0)
C           OPEN(66,FILE=FNW,ACCESS='DIRECT',
C    +         STATUS='REPLACE',RECL=4)
C           WRITE(66,REC=1) Iphr
C           CLOSE(66)
...
C     write the common-ph imaging gathers.
C     2016.04, FengBo use fortran IO function to access pwd-file.
C     write the plane-wave CIG file: FN3.
C     data structure: pdat4d(Ntau, Ncdp, Nline, Nphr).
         write(ciphr,'(I4)') Iphr
         FNCIG=trim(adjustl(FN3))//trim(adjustl(ciphr))//'.dat'
         open(IFN03, file=FNCIG, access='direct',
     +     recl=Ntau*Nmx1*Nmy1) 
         write(IFN03, rec=1) (((RIMAG(it,ix,iy),it=1,Ntau),
     +     ix=1,Nmx1),iy=1,Nmy1)
         close(IFN03)
!        IF (myid .eq. 0) THEN
...
C     Check the input time-domain layer velocity.
      open(21, file='velo_check.dat', access='direct',
     +     recl=NX*NY) 
     ...
     +        Nmx0, Nmx1, Nmy0, Nmy1)
         ...
         WRITE(21,REC=Itau)((VVtau(IX,IY),IX=1,NX),IY=1,NY)
...
C     iflag = 3
C     CALL gdbfcwriteregularfile(IFN03, ipar240, iflag, RIMAG)
C     IF (iflag .ne. 0) THEN
C         write(*,*) ' gdbfcwriteregularfile() failed.'
C         write(*,*) ' iflag = ', iflag
C         write(*,*) ' Iphr  = ', Iphr
C         STOP
C     END IF
...
C         iflag = 1
C         CALL gdbfcwriteregularfile(IFN03, ipar240, iflag,
C    +                               RIMAG(Ktaur,imx,imy))
C         IF (iflag .ne. 0) THEN
C          write(*,*) ' gdbfcwriteregularfile() failed.'
C          write(*,*) ' iflag = ', iflag
C          write(*,*) ' Iphr  = ', Iphr
...
C     IPAR240(1) = 1             ! first grid number of 1st Dimension.
C     IPAR240(2) = LT            ! last  grid number of 1st Dimension.
C     IPAR240(3) = Ncdp_first    ! location of 2nd Dimension.
C     IPAR240(5) = Iphr          ! location of 4th Dimension.
C     IPAR240(6) = 1             ! location of 5th Dimension.

C     2016.04, FengBo use fortran IO function to access pwd-file.
C     open the plane-wave gather file: FN1.
C     data structure: pdat4d(Ntau, Ncdp, Nphr, Nline).
      open(IFN01, file=trim(adjustl(FN1)), access='direct',
     +     recl=LT*ncdp) 

      DO 2010 Iline = Nline_first, Nline_final

C        IPAR240(4) = Iline      ! location of 3rd Dimension.
         Imy = Iline - Nline_first + 1 + Nmy0
C        write(*,*) 'Iline = ', Iline
         if( mod(Iline,10) .eq. 0 ) write(*,*) 'Iline= ', Iline

         irec=Iphr + (Iline - Nline_first)*Nphr
         read(IFN01, rec=irec) ((Pdat2d(it,ix),it=1,LT),ix=1,ncdp)

C        IFLAG = 2
C        CALL gdbfcreadregulardata(IFN01, IPAR240, IFLAG, Pdat2d)
C        IF (IFLAG.NE.0) THEN
C           WRITE(*,*) 'gdbfcreadregulardata() failed!'
C           WRITE(*,*) 'IFlAG = ',IFlAG
C           WRITE(*,*) 'PROGRAM EXIT.'
C           STOP
C        END IF
     ...
     +     Iphr, IFN01, Nmx0, Nmx1, Nmy0, Nmy1)
      ...
      DO Iline = Nline_first, Nline_final
         ...
         DO Icdp = Ncdp_first, Ncdp_final
            ...
            Imx = Icdp - Ncdp_first + 1 + Nmx0
C           IFLAG = 1
C           CALL gdbfcreadregulardata(IFN01, IPAR240,
C    +               IFLAG, Trace)
            IF (IFLAG.NE.0) THEN
