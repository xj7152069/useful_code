...
// MPI
#include "mpi.h"
#include "./cpp_code/slantstack3d.h"
...
double    getFnorm_2d(float **in2d, int n1, int n2)
{
    double  norm2   = 0;
    for ( int i2 = 0 ; i2 < n2; ++i2 )
        for ( int i1 = 0 ; i1 < n1; ++i1 )
            norm2   += in2d[i2][i1]*in2d[i2][i1];
    double   norm2sqrt   = sqrt(norm2);
    if (0 != norm2sqrt)
        return  norm2sqrt;
    else
        return  1.0;
        ...
        int verbose)
{
    ...
    //float   *fNorm2 = calloc(ncdp_proj, FLOAT_SIZE_BYTES);
...
//#pragma omp parallel for shared(MyCMPData_line, MycmpFileIndex_2d)
    for ( int cdpNo = cdpMin_line ; cdpNo <= cdpMax_line; ++ cdpNo )
    {
        //if (cdpNo != 1801)  continue;
        //if (cdpNo >= 1800 || cdpNo <= 1750)  continue;
...
//#pragma omp critical (read_single_CMP_gather)
        ...
        */
        ...
        {
            float   **gatherPP    = alloc2float(ns, ntrCMP);
            ...
            for ( int itr = 0 ; itr < ntrCMP; ++itr )
            {
                for( int it  = 0 ; it  < ns ; ++ it  )
                {
                    ...
                    gatherPP[itr][it] = MyCMPData_line[icdp].gather[itr][it];
                }
            }
            // Add by Xiang Jian, a median filter.
            medianfilter(gatherPP, ntrCMP, ns, 25, 1, 3);
            ...
            //write_2d_float_wb(gatherPP, ntrCMP, ns, "cdpgather_pp.dat");
            //double  frobnorm2  = getFnorm_2d(gatherPP, ns, ntrCMP);
            //fNorm2[icdp]    = (float)frobnorm2;
            ...
            */
            ...
            if ( 0 == lsinvFlag )
            {
                ...
                postProcessingTauPspectrum_diag(nphr, ntau, taup2d_frss);
                //write_2d_float_wb(taup2d_frss, nphr, ntau, "taup2d_frss_pp.dat");
                taup2d_ptr  = taup2d_frss;
            }
            else if ( 1 == lsinvFlag )
            {
                // the least-squares LRT, more accurate, at the cost of hugh computational time.
                ...
                float factor_L2=(1000.0), factor_L1=(0.0), iterations_num=(40), residual_ratio=(0.1);
                factor_L2=1.0;
                iterations_num=15;
                residual_ratio=0.15; // by Bo Feng, 2022.06.14.
                int nthread = 16;
                apply_GlobalLSLRT_to_CMPgather_omp( ntrCMP, halfoffset, ns, dt_s,
                        gatherPP, nphr, phrmin, dphr, ntau, taup2d_lslrt,
                        factor_L2, factor_L1, iterations_num, residual_ratio, nthread);
                //write_2d_float_wb(taup2d_lslrt, nphr, ntau, "taup2d_global_lslrt.dat");
                // post-processing of the lslrt result.
                postProcessingTauPspectrum_diag(nphr, ntau, taup2d_lslrt);
                //write_2d_float_wb(taup2d_lslrt, nphr, ntau, "taup2d_global_lslrt_pp.dat");
                taup2d_ptr  = taup2d_lslrt;
            }

            // store the lrt result into the 3-D buffer.
            for ( int iphr = 0 ; iphr < nphr; ++iphr )
                for ( int it = 0; it < ntau ; ++it )
                    taup3d[icdp_proj][iphr][it] = taup2d_ptr[iphr][it] ;
    ...
    */
    ...
    //write_1d_float_wb(fNorm2, ncdp_proj, "./fNorm2.bin");
    //free(fNorm2);
    free(MyCMPData_line);
    free(vrms1d);
...
int main( int argc , char *argv[] )
{
    ...
    */
    ...
    if ( 0 == myid )
        printf("\t****Run Time of Func: %s() is %f\n",  __FUNCTION__, etime-stime);
    char    taupCMPLineFilename[FILE_NAME_MAX_LENGTH]="./taup3dLine.test.dat";
    fprintf(stdout, "ntau=%d nphr=%d ncdp_proj=%d\n", ntau, nphr, ncdp_proj);
    write_3d_float_wb(taup3d, ncdp_proj, nphr, ntau, taupCMPLineFilename);
    float   ***taup3d_commph   = alloc3float(ntau, ncdp_proj, nphr);
    for ( int iphr = 0 ; iphr < nphr; ++iphr )
    {
        for ( int icdp = 0 ; icdp < ncdp_proj; ++icdp )
        {
            for( int it  = 0 ; it  < ntau ; ++ it  )
            {
                taup3d_commph[iphr][icdp][it]    = taup3d[icdp][iphr][it];
            }
        }
    }
    char    taupCMPLineFilename2[FILE_NAME_MAX_LENGTH]="./taup3dLine_commph.test.dat";
    write_3d_float_wb(taup3d_commph, nphr, ncdp_proj, ntau, taupCMPLineFilename2);
    */
    /*
    */
    /*
    char    taupCMPLineFilename[FILE_NAME_MAX_LENGTH]="./taup3dLine.lslrt.dat";
    fprintf(stdout, "ntau=%d nphr=%d ncdp_proj=%d\n", ntau, nphr, ncdp_proj);
    //write_3d_float_wb(taup3d, ncdp_proj, nphr, ntau, taupCMPLineFilename);
    read_3d_float_rb(taup3d, ncdp_proj, nphr, ntau, taupCMPLineFilename);
    float   ***taup3d_commph   = alloc3float(ntau, ncdp_proj, nphr);
    // balance the energy of each tau-p spectrum.
    double  norm2   = 0;
    for ( int icdp = 0 ; icdp < ncdp_proj; ++icdp )
    {
        for ( int iphr = 0 ; iphr < nphr; ++iphr )
            for( int it  = 0 ; it  < ntau ; ++ it  )
                norm2   += taup3d[icdp][iphr][it]*taup3d[icdp][iphr][it];
        float   norm2sqrt   = sqrt(norm2);
        if (0 == norm2sqrt)
            continue;
        for ( int iphr = 0 ; iphr < nphr; ++iphr )
            for( int it  = 0 ; it  < ntau ; ++ it  )
                taup3d[icdp][iphr][it]  /= norm2sqrt;
    }
    write_3d_float_wb(taup3d, ncdp_proj, nphr, ntau, "./taup3dLine.lslrt.taupnormdat");
    float   *fNorm2 = calloc(ncdp_proj, FLOAT_SIZE_BYTES);
    read_1d_float_rb(fNorm2, ncdp_proj, "./fNorm2.bin");
    for ( int icdp = 0 ; icdp < ncdp_proj; ++icdp )
    {
        for ( int iphr = 0 ; iphr < nphr; ++iphr )
            for( int it  = 0 ; it  < ntau ; ++ it  )
                taup3d[icdp][iphr][it]  *= fNorm2[icdp];;
    }
    write_3d_float_wb(taup3d, ncdp_proj, nphr, ntau, "./taup3dLine.lslrt.cmpnormdat");
    ...
    char    taupCMPLineFilename2[FILE_NAME_MAX_LENGTH]="./taup3dLine_commph.lslrt.frobnorm.dat";
    write_3d_float_wb(taup3d_commph, nphr, ncdp_proj, ntau, taupCMPLineFilename2);

    free3float(taup3d);
    free3float(taup3d_commph);
    */
