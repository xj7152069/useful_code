...
%% 读取数据
% single：单精度数组，节省内存
data3d=single( zeros(n2,n1,n3));
data2d=single(zeros(n3,n2));
data2d1=single(zeros(n2,n3));
data2d2=single(zeros(n2,n3));
% %打开二进制文件路径
% fid=fopen('.\data\demultiple3d.bycol.dat','rb');
fid=fopen('.\data\data3d.cut.bycol.dat','rb');
% fid=fopen('.\data\multiple.remove.bycol.dat','rb');
% fid=fopen('.\data\multiple3d.bycol.dat','rb');
    if(fid>0)
        for k=1:n1
...
%% 定义坐标网格
%分别定义三个维度上的规则网格坐标
u_1=single(n1min:d1:n1max);
u_2=single(n2min:d2:n2max);
u_3=single(n3max:-d3:n3min);
...
for sliceline=nline-1:-30:1  %沿测线方向绘制切片
% for slicetrace=1:30:92  %跨测线绘制切片
figure(sliceline)  %沿测线方向绘制切片
% figure(slicetrace)  %跨测线绘制切片
hold on
% 获取纵向（某个trace）垂直切片数据
% 需要至少两片数据 emmmm
d1=single(Z(slicetrace:slicetrace+1,:,:));
u_11=single(u_1);
u_21=single(u_2(slicetrace:slicetrace+1));
u_31=single(u_3);
...
% 获取横向垂直切片数据
d1=single(Z(:,sliceline:sliceline+1,:));
u_11=single(u_1(sliceline:sliceline+1));
u_21=single(u_2);
u_31=single(u_3);
[U_11,U_21,U_31]=meshgrid(u_11,u_21,u_31);
% 绘制横向垂直切片
...
hold on
% 绘制黑色边框线
plot3(ones(1,length(u_3))*(sliceline-1)*dline,...
    ones(1,length(u_3))*(slicetrace+1)*dtrace,u_3,'k-')
hold on
plot3(ones(1,length(u_2))*(sliceline)*dline,u_2,...
    ones(1,length(u_2))*u_3(end),'k-')
hold on
plot3(u_1,ones(1,length(u_1))*(slicetrace)*dtrace,...
    ones(1,length(u_1))*u_3(end),'k-')
hold on
% 绘制黑色边框线
xlabel('SY (km)','FontName' ,'Times New Roman', 'FontSize',20)
ylabel('SX (km)','FontName' ,'Times New Roman', 'FontSize',20)
zlabel('Time (s)','FontName' ,'Times New Roman', 'FontSize',20)
...
% 设置坐标轴范围
axis([n1min n1max n2min n2max n3min n3max*0.75])
...
% 
% %获取数据块2及其坐标
% d2=single(Z(1:p2,:,1:p3));
% u_12=single(u_1);
% u_22=single(u_2(1:p2));
% u_32=single(u_3(1:p3));
...
% 
% %获取数据块3及其坐标
% d3=single(Z(1:p2,p1:end,p3:end));
% u_13=single(u_1(p1:end));
% u_23=single(u_2(1:p2));
% u_33=single(u_3(p3:end));
